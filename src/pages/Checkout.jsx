...
const Checkout = () => {
  ...
  const [formData, setFormData] = useState(() => {
    const saved = localStorage.getItem('vive_user_profile');
    if (saved) {
      try {
  ...
  const [loyaltyPoints, setLoyaltyPoints] = useState(() => {
    const saved = localStorage.getItem('vive_loyalty_points');
  ...
  const handleProceedToPayment = (e) => {
    e.preventDefault();
    if (validateStep1()) {
      localStorage.setItem('vive_user_profile', JSON.stringify(formData));
      setActiveStep(2);
  ...
  const handlePlaceOrder = async (e) => {
    ...
    const options = {
      ...
      handler: function (response) {
        ...
        localStorage.setItem('vive_loyalty_points', finalPoints.toString());
      ...
      <main className="flex-1 max-w-[1450px] mx-auto px-4 sm:px-10 lg:px-16 py-12 md:py-16 w-full relative">
        ...
        <div className="mb-8 text-left">
          <div className="flex items-center gap-2 text-xs text-gray-400 font-bold uppercase tracking-widest mb-2">
            <Link to="/cart" className="hover:text-brand-magenta transition-colors">Bag</Link>
            <span>/</span>
